...
export function SetsEditor({ sets, onChange }: SetsEditorProps) {
  ...
  const removeSet = (index: number) => onChange(sets.filter((_, i) => i !== index));
        ...
        <div key={i} className="flex items-center gap-2">
          ...
          <NumberStepper
            ...
            onChange={(v) =>
              update(i, {
                min_reps: v,
                max_reps: set.max_reps != null && set.max_reps < v ? v : set.max_reps,
              })
            }
          />
          ...
          <span className="text-faint ml-auto shrink-0 font-mono text-[11px] uppercase">reps</span>
...
function NumberStepper({
  ...
  return (
    <div className="border-border bg-surface-2 flex h-8 items-center rounded-lg border">
      ...
      <span className={cn("tabular text-fg w-6 text-center font-mono text-sm font-semibold")}>
        {value}
      </span>
      <button
        type="button"

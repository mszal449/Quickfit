...
function topSet(weights: { weight: number | null; reps: number | null }[]): {
  ...
  let best: { weight: number | null; reps: number | null } = {
    weight: null,
    reps: null,
  };
  for (const s of weights) {
    if (s.weight == null) continue;
    if (best.weight == null || s.weight > best.weight)
...
export function buildSummary(
  ...
  const exercises: ExerciseSummary[] = model.exercises.map((ex) => {
    ...
    const previousTop = topSet(
      ex.sets.filter((s) => s.previous != null).map((s) => s.previous!),
    );
    const isPr =
      top.weight != null &&
      previousTop.weight != null &&
      top.weight > previousTop.weight;
    if (isPr) prCount += 1;
